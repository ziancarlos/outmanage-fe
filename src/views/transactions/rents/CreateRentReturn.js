...
function CreateRentShipment() {
  ...
  async function fetchTransactionInventories(transactionRentId) {
    try {
      ...
      inventoryOptionsRef.current = response.data.data.map((item) => ({
        ...
        label: `${item.inventory.name} | ${item.inventory.condition === 0 ? 'BARU' : 'BEKAS'} | Kuantitas Tersedia Dibalikkan: ${Number(item.availableRentQuantity)}`,
  ...
  function handleAddItem(e) {
    ...
    if (inventoryValue?.availableRentQuantity < Number(quantityValue)) {
      setError('Jumlah barang yang ingin dikirim melebihi kuantitas yang tersedia')
      return
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      await axiosPrivate.post(`/api/transactions/rents/${transactionRentId}/returns`, request)

      Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: 'Proses Pengembalian Transaksi pembelian berhasil dibuat.',
      ...
      {loading ? (
        ...
        <CRow className="mb-3">
          <CCol>
            <CCard>
              <CCardHeader>
                <strong>Tambah Transaksi Penyewaan Pengembalian</strong>
              </CCardHeader>
              <CForm onSubmit={handleSubmit}>
                <CCardBody>
                  ...
                  <div className="mb-3">
                    <CFormLabel className="fw-bold">
                      Alamat Penjemputan <CBadge color="info">Optional</CBadge>
                    </CFormLabel>
                    <CFormTextarea
                      rows={3}
