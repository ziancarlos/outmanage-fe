...
export default function TableLogCustomer({
  title = 'Data Log Kustomer',
  customerId = null,
  size = 4,
  ...
  useEffect(() => {
    ...
    const queryParams = searchParams.get('customersLogs')
  ...
  async function fetchCustomer() {
    try {
      ...
      const response = await axiosPrivate.get('/api/customers/logs', { params })

      setCustomersLogs(response.data.data)
  ...
  function handlePageChange(newPage) {
    if (newPage >= 1 && newPage <= totalPages && newPage !== page) {
      ...
      newParams.set('customersLogs', JSON.stringify(filterRef.current))
  ...
  async function handleSearch(e) {
    ...
    if (Object.keys(filterRef.current).length > 0) {
      ...
      newParams.delete('customersLogs')
    }
                  ...
                  {customersLogs.map((log) => {
                    const customer = canReadCustomer ? (
                      log.customer?.customerId ? (
                        <NavLink to={`/customers/${log.customer.customerId}/detail`}>
                          {log.customer.name}
                        </NavLink>
                      ) : (
                        '-'
                    ...
                    return (
                      <CTableRow key={log.customerLogId}>
                        <CTableDataCell>CL{log.customerLogId}</CTableDataCell>
