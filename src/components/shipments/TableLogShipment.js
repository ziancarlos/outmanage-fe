...
export default function TableLogShipment({
  ...
  const [shipmentsLogs, setShipmentsLogs] = useState([])
  ...
  useEffect(() => {
    ...
    const queryParams = searchParams.get('shipmentsLogs')
    ...
    fetchShipment(filterRef.current).finally(() => {
      setLoading(false)
    })
  }, [refetch])
  ...
  function handlePageChange(newPage) {
    if (newPage >= 1 && newPage <= totalPages && newPage !== page) {
      ...
      newParams.set('shipmentsLogs', JSON.stringify(filterRef.current))
  ...
  async function handleSearch(e) {
    ...
    if (Object.keys(filterRef.current).length > 0) {
      ...
      newParams.delete('shipmentsLogs')
    }
              ...
              <CTable striped bordered responsive>
                ...
                <CTableBody>
                  {shipmentsLogs.map((log) => {
                    ...
                    return (
                      <CTableRow key={log.shipmentLogId}>
                        <CTableDataCell>SL{log.shipmentLogId}</CTableDataCell>
                        <CTableDataCell>{shipment}</CTableDataCell>
