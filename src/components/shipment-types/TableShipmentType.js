...
function TableShipmentType({
  title = 'Data Tipe Pengiriman',
  authorizePermissions,
  endpoint = '/api/shipment-types',
  ...props
}) {
  const canReadShipmentType = authorizePermissions.some(
    (perm) => perm.name === 'read-shipment-type',
  )
  const canUpdateShipmentType = authorizePermissions.some(
    (perm) => perm.name === 'update-shipment-type',
  )
  ...
  useEffect(() => {
    ...
    const queryParams = searchParams.get('shipmentTypes')
  ...
  function handlePageChange(newPage) {
    if (newPage >= 1 && newPage <= totalPages && newPage !== page) {
      ...
      newParams.set('shipmentTypes', JSON.stringify(filterRef.current))
  ...
  async function handleSearch(e) {
    ...
    if (Object.keys(filterRef.current).length > 0) {
      ...
      newParams.delete('shipmentTypes')
    }
  ...
  function handleDetail(shipmentTypeId) {
    navigate(`/shipment-types/${shipmentTypeId}/detail`)
  }

  function handleUpdate(shipmentTypeId) {
    navigate(`/shipment-types/${shipmentTypeId}/edit`)
  }
              ...
              <CTable striped bordered responsive>
                <CTableHead>
                  <CTableRow>
                    ...
                    {(canReadShipmentType || canUpdateShipmentType) && (
                ...
                <CTableBody>
                  {shipmentTypes.map((type) => {
                      ...
                      <>
                        {canReadShipmentType && (
                          <CButton
                            color="info"
                            size="sm"
                            onClick={() => handleDetail(type.shipmentTypeId)}
                          >
                        ...
                        {canUpdateShipmentType && (
                          <button
                            ...
                            onClick={() => handleUpdate(type.shipmentTypeId)}
                          >
                    ...
                    return (
                      <CTableRow key={type.shipmentTypeId}>
                        <CTableDataCell>ST{type.shipmentTypeId}</CTableDataCell>
...
export default TableShipmentType
